...
def make_move(player_turn, position, pieces, selected, aux_pos, color):
    ...
    if (selected == position.poss1 and position.busy == False) or (selected == position.poss2 and position.busy == False) or (selected == position.poss3 and position.busy == False):
...
def cpu_i(n_play, player_turn, positions, black_pieces, white_pieces, PLAY_MOUSE_POS, POSITIONS):
    if n_play >= 5:
        for i in positions:
